...
/**
 * CI Selective Routing — MAIN CI PIPELINE
 *
 * New file submitted with UPDATED config (ARC status → CI role).
 * File: TODO-submit-file-on-main
 * Process: TODO-submit-file-on-main
...
const SS = path.resolve(__dirname, '../screenshots/main-ci-pipeline');
const SERVICE_ID = '0d8ca0c6-2a74-4d10-8c64-0b4e4a186adc';
const PROCESS_ID = '3f3e03a2-18fc-11f1-899e-b6594fb67add';
    ...
    await page.evaluate(() => {
      ...
      for (const k of Object.keys(formio.forms)) {
        ...
        form.root.everyComponent((comp: any) => {
          ...
          // DocCheck on MAIN requires agency approval checkboxes
...
async function processRole(page: any, camundaName: string, testPdf: string): Promise<boolean> {
  ...
  // DocCheck on MAIN: button is disabled (conditional blocks it).
  // comp.onClick() works — it triggers PUT /backend/process/.../role-forms/documentsCheck/WIZARD
  // which advances the task. customEvent emit does NOT work for this button.
  if (camundaName === 'documentsCheck') {
    console.log('      DocCheck: triggering via comp.onClick() (force-enable)...');
    await page.evaluate(() => {
      const formio = (window as any).Formio; if (!formio?.forms) return;
      for (const fk of Object.keys(formio.forms)) {
        const form = formio.forms[fk]; if (!form?.root) continue;
  ...
  // Priority order matters! More specific selectors first.
  // "Send to Board" removed — only used in Phase F (ARC) which is handled separately.
  // "Approve" must come LAST since it's a substring of "Approve and send to ARC".
  const btnSelectors = [
    ...
    'button:has-text("Send SEZ")',
    'button:has-text("Send documents")',
    // "Approve" is btn-block (full-width system action button) — match it specifically
  ...
  // Board submission: fill vote + risk analysis + agency checkboxes
  if (camundaName === 'boardSubmission') {
    console.log('      Board: filling vote, risk, checkboxes...');
    await page.evaluate(() => {
      const formio = (window as any).Formio; if (!formio?.forms) return;
      for (const fk of Object.keys(formio.forms)) {
        const form = formio.forms[fk]; if (!form?.root) continue;
        form.root.everyComponent((comp: any) => {
          ...
          if (key === 'boardSubmissionVoteChairman') { comp.setValue('Approved'); comp.triggerChange?.(); }
          if (key === 'boardSubmissionRiskAnalysis') { comp.setValue('Low'); comp.triggerChange?.(); }
          if (key === 'boardSubmissionNonObJca') { comp.setValue(true); comp.triggerChange?.(); }
          if (key === 'boardSubmissionNonObJca2') { comp.setValue(true); comp.triggerChange?.(); }
          if (key === 'boardSubmissionNonObJca3') { comp.setValue(true); comp.triggerChange?.(); }
        });
      }
    });
    await page.waitForTimeout(2000);
...
test('CI Pipeline — MAIN — New File', async ({ page }) => {
  ...
  console.log('\n╔══════════════════════════════════════════════════╗');
  console.log('║  CI SELECTIVE ROUTING — MAIN CI PIPELINE         ║');
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    // Guard: if same task already attempted twice, it's stuck
    const prevAttempts = completedRoles.filter(r => r === task.camundaName).length + skippedRoles.filter(r => r === task.camundaName).length;
    if (prevAttempts >= 2) {
      console.log(`      ❌ Task "${task.camundaName}" stuck after ${prevAttempts} attempts. Stopping.`);
      break;
    }
  ...
  if (!reachedARC) {
    ...
    fs.writeFileSync(path.resolve(__dirname, '../test-data/main-ci-results.json'), JSON.stringify(report, null, 2));
    return;
  }
  ...
  if (arcBack) {
    console.log('\n═══ PHASE F: ARC → Send to Board submission ═══\n');
    // Proven approach from PARTB ci-phase-f-board.spec.ts:
    // 1. Set radio "no" → unhides Board button
    // 2. Save/cancel EditGrid rows (validation blocks buttons)
    // 3. Save draft
    // 4. DOM click on Board button
    ...
    // Step 1: Set additional info = no + FORMDATAVALIDATIONSTATUS = true
    console.log('  Step 1: Setting radio to "no"...');
    await page.evaluate((key: string) => {
      const formio = (window as any).Formio; if (!formio?.forms) return;
      for (const fk of Object.keys(formio.forms)) {
        const form = formio.forms[fk]; if (!form?.root) continue;
    ...
    // Step 2: Save/cancel unsaved EditGrid rows
    console.log('  Step 2: Fixing EditGrid rows...');
    const gridFixF = await page.evaluate(() => {
    ...
    console.log(`    Saved: ${gridFixF.saved}, Cancelled: ${gridFixF.cancelled}`);
    ...
    // Step 4: DOM click on Board button
    ...
    if (isBoardVisible) {
      ...
      // Fallback: try enableAndClick with force
      console.log('  ⚠ Board button not visible, trying enableAndClick...');
      await enableAndClick(page, boardBtn);
    }

    await ss(page, 'f-after-board');
    await page.goto('/'); await page.waitForTimeout(5000);
    tasks = await getTasks(page); pNames = pendingNames(tasks);
    const arcAdvancedF = !pNames.includes(ARC_ROLE);
    console.log(`  After Board: pending = ${pNames.join(', ')}`);
    console.log(`  ARC advanced: ${arcAdvancedF ? '✅ YES' : '❌ NO'}`);
    report.steps.push({ phase: 'F', arcAdvanced: arcAdvancedF, pendingTasks: pNames });
  }

  // ════════════════════════════════════════════
  // PHASE G: Process remaining roles → completion
  // ════════════════════════════════════════════
  console.log('\n═══ PHASE G: Process remaining roles ═══\n');

  let gIteration = 0;
  const MAX_G_ITERATIONS = 20;
  while (gIteration < MAX_G_ITERATIONS) {
    gIteration++;
    tasks = await getTasks(page);
    pNames = pendingNames(tasks);
    if (pNames.length === 0) { console.log('  🏁 No more pending tasks!'); break; }
    console.log(`\n  [G-${gIteration}] Pending: ${pNames.join(', ')}`);

    let processed = false;
    for (const name of pNames) {
      // Skip ARC if it's still stuck (shouldn't happen)
      if (name === ARC_ROLE) continue;
      const ok = await processRole(page, name, testPdf);
      if (ok) { processed = true; break; }
    }
    if (!processed) {
      console.log('  ⚠ No role could be processed. Stopping.');
      break;
    }
    await page.waitForTimeout(2000);
  }
  ...
  fs.writeFileSync(path.resolve(__dirname, '../test-data/main-ci-results.json'), JSON.stringify(report, null, 2));
  console.log('\n  Results saved to test-data/main-ci-results.json');
});
