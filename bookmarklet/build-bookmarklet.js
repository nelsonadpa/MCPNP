...
// 2. Collect all presets from er-presets/
const presets = {};
...
// 3. For the built file (served version), embed presets
let srcWithPresets = src;
if (presetCount > 0) {
  srcWithPresets = src.replace(
    "const EMBEDDED_PRESETS = '__EMBEDDED_PRESETS__';",
    `const EMBEDDED_PRESETS = ${JSON.stringify(presets)};`
  );
  console.log(`  ${presetCount} preset(s) for ${serviceCount} service(s)`);
} else {
  console.log('  No presets found in er-presets/');
}
// For the inline bookmarklet, never embed presets (too large) — presets come from seed

// 4. Write the full JS file with presets (for served version)
fs.writeFileSync(path.join(__dirname, 'er-filler.built.js'), srcWithPresets);

// 5. Generate a seed-presets.js that can be pasted in console to seed localStorage
const seedScript = `(function(){
  ...
  console.log('eR Filler: seeded '+seeded+' preset(s)');
  alert('Presets loaded: '+seeded+' preset(s) for '+Object.keys(presets).length+' service(s). Click the bookmarklet now.');
})();`;
fs.writeFileSync(path.join(__dirname, 'seed-presets.js'), seedScript);

// 6. Generate install.html with TWO bookmarklets:
...
if (useLoader) {
  ...
  fillerBookmarklet = "javascript:(function(){var s=document.createElement('script');s.src='http://localhost:7788/er-filler.built.js';document.head.appendChild(s);})()";
  console.log('  Source too large for inline bookmarklet, using loader approach');
  console.log('  Run: node bookmarklet/serve.js   (serves on localhost:7788)');
} else {
...
<div class="c">
  <h1>eR Form Filler</h1>
  <p class="sub">Fill eRegistrations forms instantly with AI-generated presets</p>
  <div class="stats">
    <div class="stat"><div class="stat-n">${serviceCount}</div><div class="stat-l">Services</div></div>
    <div class="stat"><div class="stat-n">${presetCount}</div><div class="stat-l">Presets</div></div>
  </div>
  ...
  <div class="step">
    ...
    ${presetCount > 0 ? `<p>
      <a class="bm seed" href="${seedBookmarklet}">Seed Presets (${presetCount})</a>
  ...
  <div class="step">
    ...
    <p>Open <strong>any eRegistrations page</strong> and click <strong>"Seed Presets"</strong> once. This loads ${presetCount} preset(s) into your browser's localStorage. You only need to do this once (or after updates).</p>
  </div>

  <div class="step">
    <span class="sn">3</span><h2>Use: Fill forms instantly</h2>
    <p>Open a service form, click <strong>"eR Form Filler"</strong>, select scenario, click <strong>Fill Form</strong>.</p>
  </div>

  ${presetCount > 0 ? `<div class="step">
    <span class="sn">i</span><h2>Pre-built presets</h2>
  ...
  <div class="footer">eR Form Filler v2.1 - Built ${new Date().toISOString().slice(0,10)}</div>
</div></body></html>`;

fs.writeFileSync(path.join(__dirname, 'install.html'), html);

// 9. If loader approach, generate serve.js
if (useLoader) {
...
console.log('\n  Build complete:');
console.log(`   er-filler.built.js          (${(src.length/1024).toFixed(1)} KB)`);
